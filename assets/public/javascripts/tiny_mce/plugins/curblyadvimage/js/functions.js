function select_image(element){
	element = $(element);
	element.parents('ul').children('li').removeClass("selected");
...
function select_thumb(element) {
    ...
    img = element.parents('li').children('a').children('img');
...
function upload_image_callback(url, alt_text, photo_id){
	$('#image_uploaded_data').html("");
    jQuery.ajax({dataType:'script', type:'get', url:'/tiny_mce_photos/?page=1'});
}
